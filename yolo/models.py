...
from yolo.utils import build_targets
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        prediction = x.view(bs, self.num_anchors, self.bbox_attrs,
                            g_dim, g_dim).permute(0, 1, 3, 4, 2).contiguous()
        ...
        if targets is not None:
            ...
            # Mask outputs to ignore non-existing objects (but keep confidence
            # predictions)
            ...
            loss_conf = self.bce_loss(conf * mask, mask) + \
                self.lambda_noobj * \
                self.bce_loss(conf * (1 - mask), mask * (1 - mask))
            loss_cls = self.bce_loss(pred_cls[mask == 1], tcls[mask == 1])
            loss = loss_x + loss_y + loss_w + loss_h + loss_conf + loss_cls

            return loss, loss_x.item(), loss_y.item(), loss_w.item(), \
                loss_h.item(), loss_conf.item(), loss_cls.item(), float(nCorrect / nGT)

        else:
            # If not in training phase return predictions

            output = torch.cat((pred_boxes.view(
                bs, -1, 4) * stride, conf.view(bs, -1, 1), pred_cls.view(bs, -1, self.num_classes)), -1)
            return output.data
...
class Darknet(nn.Module):
    ...
    def load_weights(self, weights_path):
        """Parse and loads the weights stored in 'weights_path'"""
